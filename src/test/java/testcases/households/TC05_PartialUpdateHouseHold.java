...
public class TC05_PartialUpdateHouseHold extends TestBase {
    ...
    @Test(priority =1 ,description = "Partial Update HouseHold")
    public void verifyPartialUpdateHouseHold_P() throws JsonProcessingException {
        response = given().auth().basic("admin","admin").header("Content-Type","application/json")
                .header("G-Token","ROM831ESV")
                .body("{ \"name\": \""+fullName+"\"\n" + "}")
                .when().patch("/households/"+houseHoldId).then().extract().response();
