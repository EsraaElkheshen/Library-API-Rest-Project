...
public class TC05_PartialUpdateWishList extends TestBase {
    ...
    @Test(priority = 1, description = "Verify Partial Update WishList")
    public void verifyPartialUpdateWishList_P() {
        ...
        response = given().auth().basic("admin", "admin").header("Content-Type", "application/json")
                ...
                .when().patch("/wishlists/"+wishListId).then().extract().response();
