...
public class TC01_CreateNewWishList extends TestBase {
    ...
    @Test(priority =1 ,description = "Create New WishList")
    public void verifyCreateNewWishList_P(){
        JSONObject requestBody = new JSONObject();
        JSONArray arr = new JSONArray();
        requestBody.put("name", name);
        requestBody.put("books",arr);
        response = given().log().all().auth().basic("admin","admin").header("Content-Type","application/json")
                .header("G-Token","ROM831ESV")
                .body(requestBody.toJSONString())
                .when().post("/wishlists").then().log().all().extract().response();
